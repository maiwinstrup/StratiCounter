...
if strcmp(Model.icecore,'SyntheticData')
    ...
    while isnan(meanLambda) || meanLambda <= 0
        disp(['There are no manual layer counts provided for interval. '...
            'Provide a rough estimate for mean layer thickness (in m) '...
...
if Runtype.plotlevel > 0
    ...
    colours = [0.5 0.5 0.5; 0 0 1; 0 1 0; 1 0 0];
    filename = [outputdir '/manuallayertemplates.jpeg'];   
    hfig_template = plotlayertemplates(Template0,...
            Template0Info,Model,nan,colours(1:Model.order+1,:),filename,'manual');
end
...
%% Number of batches within depth interval:
% Estimated value:
...
% May use a smaller number of batches:
if strcmp(Runtype.develop,'yes')
...
%% Iterating over layer templates:
disp('Algorithm is running, please be patient...')

for iTemplateFull = 1:Model.nTemplate

    %% Initialize arrays and set initial conditions:
    if iTemplateFull == 1
        [batchStart,Layer0,Template,Prior,Layerpar,dDxLambda,logPobs,...
            logPobsNorm,relweight,Result] = ...
            setinitialconditions(Data,Model,manualcounts,Template0,...
            Layerpar0,nBatch);
        
    else
        disp(['Restarting from beginning of the data series using '...
            'the derived templates (iteration #' num2str(iTemplateFull) ')'])
        
        [batchStart,Layer0,Template,Prior,Layerpar,dDxLambda,logPobs,...
            logPobsNorm,relweight,Result] = ...
            setinitialconditions(Data,Model,manualcounts,Template0_new,...
            Layerpar0_new,nBatch);
        % The manualcounts are here only used to provide the location of 
    ...
    while iBatch < nBatch    
        ...
        % And provide an upper-bound estimate of the number of layers in 
        % batch.
        if isempty(Model.tiepoints)
            % Situation A: No tiepoints.
            % We select a section of appropriate length. This section may 
            % be larger than nLayerBatch if close to the end of data series.
            ...
            % In case there is less left than a full batch of data, the 
            % current batch is extended to include these extra pixels:
        ...
        % Data in batch is preprocessed relative to the mean layer 
        % thickness as estimated from previous batch.
        % Preprocessing specs:
        ...
        %% 2+3: Iterations over layer template and layer parameters for batch
        % The layer parameters are first optimized according to the initial
        % template. Then the layer template is optimized, and layer 
        % parameters are re-optimized for this new template, etc.

        % Initial layer template for batch:
        % The same initial template is used for all batches. 
        % This initial template may further be iterated over, based on the
        % results from the entire data series, according to the value of 
        % Model.nTemplate.
        Template(:,iBatch,1)=Template(:,1,1);

        % Iterations over batchwise layer template:
        for iTemplateBatch = 1:Model.nTemplateBatch
            ...
            if Runtype.plotlevel > 1
                colours_new = [1 0 1; 1 1 0; 1 0.5 0.5; 0 0 1];
                filename = [outputdir '/layertemplates_new'];
                plotlayertemplates(Template_new,TemplateInfo_new,Model,...
                    hfig_template,colours_new,filename,'new');                
            end
            
            % Possibility for updating the layer templates:
            % Currently, the initial template for each batch is always the
            % same, as the below setting is overridden by code in line 375.
%             Template(:,iBatch,iTemplateBatch+1)=Template_new;
...
%             end
        ...
        % Results for batch: layer probability distribution, the most 
        % likely layer boundaries, and initial conditions for next batch.
        ...
        % Calculate log(P_obs) for entire data series, up to and including 
        % this one:
        ...
        if iBatch==nBatch
            ...
            % If running in develop mode, we may have provided a maximum 
            % batch number:
            if strcmp(Runtype.develop,'yes')
            ...
            if nBatchRest > 0
                [batchStartRest,Layer0Rest,TemplateRest,PriorRest,...
                    LayerparRest,logPobsRest,logPobsNormRest,...
                    relweightRest,ResultRest]=...
    ...
    %% Save output:
    % Timescale data files:
    filename1 = [outputdir '/timescale'];
    filename2 = [outputdir '/' Model.icecore '_timescale_' ...
        num2str(Model.dstart) '-' num2str(Model.dend) 'm'];
    if iTemplateFull~=Model.nTemplate 
        filename1 = [filename1 '_template' num2str(iTemplateFull)];
        filename2 = [filename2 '_template' num2str(iTemplateFull)];
    end
    % Save as matlab data files:
    save([filename1 '.mat'],'timescale','timescale1yr','Layerpos','LayerProbDist','centralEst','Model')
    % Save timescale1yr as textfile with metadata:
    savetimescaleastxt(timescale1yr,[filename2 '.txt'],Model)

    % Confidence interval for marker horizons:
    filename1 = [outputdir '/markerhorizons'];
    filename2 = [outputdir '/' Model.icecore '_markerconf'];
    if iTemplateFull~=Model.nTemplate 
        filename1 = [filename1 '_template' num2str(iTemplateFull)];
        filename2 = [filename2 '_template' num2str(iTemplateFull)];
    end
    if ~isempty(Model.Out.dMarker)
        save([filename1 '.mat'],'markerProb','markerConf','Model')
        % As txt file:
        savemarkerastxt(markerConf,filename2,Model)
    end

    % Layer thicknesses:
    filename = [outputdir '/lambda'];
    if iTemplateFull~=Model.nTemplate 
        filename = [filename '_template' num2str(iTemplateFull)];
    end
    if ~isempty(Model.Out.dxLambda)
        save(filename,'lambdaResults','Model')
    end

    % Save all results from iterations (!):
    filename = [outputdir '/results'];
    if iTemplateFull~=Model.nTemplate 
        filename = [filename '_template' num2str(iTemplateFull)];
    end
    save([filename '.mat'],'Result','relweight','logPobs','logPobsNorm',...
    ...
    if Runtype.plotlevel>0
        % Timescale:
        if exist('hfig_timescale','var'); close(hfig_timescale); end
        filename = [outputdir '/timescale'];
        if iTemplateFull~=Model.nTemplate
            filename = [filename '_template' num2str(iTemplateFull)];
        end
        hfig_timescale = showtimescale(timescale,timescale1yr,...
            manualcounts,Data.depth(batchStart),Model,[filename '.jpeg']);

        % Mean layer thicknesses:
        if exist('hfig_lambda','var');
            for i = 1:length(Model.Out.dxLambda)
                if isgraphics(hfig_lambda(i))
                    close(hfig_lambda(i));
                end
            end
        end
        hfig_lambda = gobjects(length(Model.Out.dxLambda),1); % Initialize handles
        for idx = 1:length(Model.Out.dxLambda)
            filename = [outputdir '/lambda_' num2str(Model.Out.dxLambda(idx)) 'm'];
            if iTemplateFull~=Model.nTemplate
                filename = [filename '_template' num2str(iTemplateFull)];
            end
            hfig_lambda(idx) = showlambda(lambdaResults{idx},timescale1yr,...
                manualcounts,Model,[filename '.jpeg']);
        end
    end

    %% Calculate new layer templates:
    % Based on data in the entire interval, for which the algorithm has
    % run.

    % In case of floating preprocessing steps, this must be done first, and 
    % is then based on an average value of lambda over interval.
    % Preprocessing specs:
    ...
    % If performing full template iterations, these are used as new initial 
    ...
    % Plot new layer templates (and compare to original ones):
    if Runtype.plotlevel > 0
        colours(1,:) = colours(1,:)*0.7;
        colours(2:end,:) = colours(2:end,:)*0.5;
        filename = [outputdir '/layertemplates_new'];
        if iTemplateFull~=Model.nTemplate
            filename = [filename '_template' num2str(iTemplateFull)];
        end
        displayname = ['after iteration #' num2str(iTemplateFull)];
        plotlayertemplates(Template_new,TemplateInfo_new,Model,...
            hfig_template,colours,filename,displayname);
    end
end
