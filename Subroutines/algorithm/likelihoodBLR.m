...
obs = [NaN(dmax,length(Model.deriv),Model.nSpecies); data(:,Model.deriv+1,:); ...
    NaN(dmax,length(Model.deriv),Model.nSpecies)];
...
if strcmp(Model.normalizelayer,'minusmean')
    ...
    obs_segment = obs_segment - meansignal(:,Model.deriv+1,:);
end

%% Reshape array, as we consider the signal and its derivatives to be 
% modelled with the same parameter values:
obs_segment = reshape(obs_segment,length(Model.deriv)*d,Model.nSpecies);
...
for j = 1:Model.nSpecies
    ...
    w = ones(d,1)*Model.wWhiteNoise(Model.deriv+1);
    w = w(:); 
    w = w(mask);
    W = diag(w);
    % And its inverse:
